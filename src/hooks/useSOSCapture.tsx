...
export function useSOSCapture() {
  ...
  /**
   * Notify all emergency contacts by creating a safety_alert record
   * and (on supported platforms) opening an SMS to their phone number
   * with the user's location + message.
   */
  const notifyContacts = async (opts: {
    ...
    // Create a safety_alert record for each contact (stored in DB)
    ...
    // Open SMS on first/primary contact (browser limitation — can only open one)
    const primary = contacts[0];
    if (primary?.contact_phone) {
      const smsBody = encodeURIComponent(message);
      const smsUrl = `sms:${primary.contact_phone}?body=${smsBody}`;
      // Open in background — won't navigate away
  ...
  const triggerSOS = useCallback(
    async (opts: { location?: { latitude: number; longitude: number } | null; notes?: string }) => {
      ...
      // Run evidence captures in parallel; failures are non-fatal
  ...
  // Best-effort sync of any queued offline SOS events
...
export function useSOSCapture() {
  ...
  const triggerSOS = useCallback(
    async (opts: { location?: { latitude: number; longitude: number } | null; notes?: string }) => {
      ...
      // Run captures in parallel; failures are non-fatal
      const [audioBlob, photoBlob] = await Promise.all([captureAudio(), capturePhoto()]);
